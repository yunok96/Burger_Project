...
public class NewOrder : MonoBehaviour
{
    ...
    float curCustomerSpawnTime;
    float maxCustomerSpawnTime;
    ...
    void Start()
    {
        maxCustomerSpawnTime = 5f;//최초 스폰 타임
    }

    void Update()
    {
        curCustomerSpawnTime += Time.deltaTime * gm.worldTime;
        if (curCustomerSpawnTime > maxCustomerSpawnTime)
        {
            for(int i = 0; i < isTableOnOrder.Length; i++)
            {
                if (!isTableOnOrder[i])
                {
                    ...
                    switch (i)
                    {
                        case 0:
                            {
                                customerSpec[i].GetComponent<CustomerMove>().targetTable = new Vector3(-12, 3.5f, 0);
                                customerSpec[i].GetComponent<CustomerMove>().whoIsMe = 0;
                            }
                            break;
                        case 1:
                            {
                                customerSpec[i].GetComponent<CustomerMove>().targetTable = new Vector3(-12, 2.5f, 0);
                                customerSpec[i].GetComponent<CustomerMove>().whoIsMe = 1;
                            }
                            break;
                        case 2:
                            {
                                customerSpec[i].GetComponent<CustomerMove>().targetTable = new Vector3(-12, 1.5f, 0);
                                customerSpec[i].GetComponent<CustomerMove>().whoIsMe = 2;
                            }
                            break;
                        case 3:
                            {
                                customerSpec[i].GetComponent<CustomerMove>().targetTable = new Vector3(-12, -0.5f, 0);
            ...
            curCustomerSpawnTime = 0f;
            maxCustomerSpawnTime = 5f;//이후 기본 스폰 타임
        }

        if (Input.GetKeyDown(KeyCode.Return) && whereIsPlayer!=0)
        {
