...
public class EnemyMovement41 : MonoBehaviour
{
    float movespeed;
    ...
    void Update()
    {
        ...
        if (Vector3.Distance(transform.position, movePoint.position) <= .05f)//무브포인터와 본체의 거리가 가까우면
        {
            ...
            {
                if (movePoint.position.y != target.position.y)
                    MoveUD();
                else if (movePoint.position.x!=target.position.x)
                    MoveLR();
            }
        }
    }
    ...
    void MoveUD()
    {
        if (movePoint.position.y > target.position.y)//무브포인터의 x축보다 타겟의 x축이 작다면(아래쪽) 
        {
        ...
        else if (movePoint.position.y < target.position.y)//무브포인터의 y축보다 타겟의 y축이 크다면(위쪽) 
        {
            if (!Physics2D.OverlapCircle(movePoint.position + new Vector3(0f, 1f, 0f), .2f, WhatStopMove))//무브포인터가 이동하는 방향에 지정한 레이어가 없다면
            {
                movePoint.position += new Vector3(0f, 1f, 0f);//무브포인터 이동
