...
public class DialogueMaker : MonoBehaviour
{
    ...
    void GenerateData()//규칙. "텍스트 : 왼쪽 초상화 (0미출력) : 오른쪽 초상화 : 암전 효과 (0없음, 1왼쪽만 암전, 2오른쪽만 암전, 3양쪽 암전) : 0이면 효과없음, 1이면 카메라 흔들림(비우면 안됨)"  특수 규칙. "Event로 하면 조작 불가능인 상태로 대화창만 사라짐 :숫자 로 이벤트 설정"
    {
        ...
        talkData.Add(2325, new string[] {
            //이벤트 후
            "주인공: 따라 갈게요.:28:15:1:0",
            "퇴마사: 잘 생각했다.:28:15:2:0",
            "주인공: 제발… 이 악몽을 끝내 주세요.:28:17:1:0",
            "퇴마사: 눈을 감고 내 손을 놓지 마라.:28:17:2:0",
            "퇴마사: 보기 좋지 못할 광경이 벌어질거니:28:17:2:0"
            //이벤트 시작
        });
        talkData.Add(2326, new string[] {
            ...
            "???: 그렇게 피가 싫었어? (cg:일반)",
            "주인공: 누구세요? (cg:진지)",
            "???: 이제 나를 모른 척 하는 거야? (cg:일반)",
            "주인공: 저는 당신을 몰라요(cg:진지)",
            "주인공: 당신이… 설마 그 악신? (cg:진지)",
            "???: 악신이라니, 너무 서운하게 만드는 걸. (cg:일반)",
            "???: 우리가 처음 만났을 때는 분명(cg:일반)",
            "???: 너가 나를 꼬셨잖아(cg:일반)",
            "주인공: 뭐… 라고요? (cg:놀람)",
            "???: 피에 대한 호기심에 미쳐서(cg:일반)",
            "???: 먼저 손을 내밀었던 것은 (cg:일반)",
            "???: 너였지. (cg:일반)",
            "???: 너는 내게 몸과 영혼을 맡겼고(cg:일반",
            "???: 우리는 매일같이 행복의 만찬을 누렸어.(cg:일반",
            "???: 일 년 전에도, 한 달 전에도… 바로 어제도.(cg:일반",
            "???: 전혀 기억하지 못하는 거야? (cg:일반",
            "주인공: 그런 기억 없어요…(cg:놀람",
            "주인공: …그런 것은 내가 아니야!!!! (cg:분노",
            "???: 아니라고 부정해도 소용 없어…(cg:일반",
            "???: 너는 ‘제물’ 먹었다고(cg:일반)",
            "???: 식욕을 주체할 수 없지 않아? (cg:일반",
            "주인공: 아니라고!!!! (cg:분노",
            "주인공: 그만 나를 놓아줘!!!!! (cg:분노",
            "???: …(cg:실망",
            "???: 돌아오지 않는 건가…(cg:실망",
            "???: 그 쥐새끼 녀석이 너를 망가뜨렸구나(cg:실망",
            "???: 난 여전히 너가 좋은데…(cg:실망",
            "주인공: 나한테 말 걸지 마(cg:분노",
            "주인공: 나는 이 지옥을 벗어날 거야(cg:분노",
            "주인공: 제발 내게서 떨어지라고!!!! (cg:분노",
            "???: …(cg:실망",
            "???: 멋대로 나를 끌어들이고서는(cg:실망",
            "???: 이제 와서 내게 멀어지려 하다니(cg:실망",
            "???: …. (cg:실망",
            "???: 알겠어(cg:일반",
            "???: 너를 돌려 놓을 수는 없는 거네.(cg:일반",
            "???: …하지만 너를 포기 할 수도 없지",
            "주인공: 갑자기 무슨 소리를 하는거야? (cg:놀람",
            "???: 너를 먹을거야(cg:일반",
            "???: 그러면 내가 너를 좀 더 이해 할 수 있겠지(cg:일반",
            "주인공: 먹는다니 무슨소리야 (cg:놀람",
            "주인공: 그만둬!!!!!! (cg:분노",
            "???: 걱정하지마. 고통스럽지 않아. (cg:일반",
            "???: 하찮은 미물로 사는 것 보다(cg:일반",
            "???: 신의 일부가 되어 사는것은(cg:일반)"
            //이벤트 시작
        });
